...
async function analyzePath(){
  ...
  try {
    const response = await fetch("/api/learning-path", {
      ...
      body: JSON.stringify({learning_goal: appState.profile.goal || "Learn a new subject"})
    });
    if (!response.ok) throw new Error("API request failed");
    const data = await response.json();
    setAIState("responding");
    if (data.success && data.learning_path) {
      appState.learningPath = data.learning_path;
      toast("Personal learning path generated successfully.");
      renderTab("learning");
    }
  } catch (error) {
    console.error(error);
    setAIState("idle");
    toast("Could not connect to the learning path API.");
  }
...
function renderTab(tab=appState.tab){
  ...
  if(tab==='learning'){
    const path=appState.learningPath||[];
    if(path.length){
      box.innerHTML=`<div class="learning-layout">${path.map(step=>`<div class="content-box"><h3>Stage ${step.stage}: ${escapeHTML(step.title)}</h3><p>${escapeHTML(step.description)}</p><div class="chips"><span class="chip">${step.progress}% complete</span></div></div>` ).join('')}</div>`;
    }else{
      box.innerHTML=`<div class="learning-layout"><div class="content-box"><h3>Learning path not generated yet</h3><p>Complete your profile and let the AI create your personalized path.</p></div></div>`;
    }
  }
  if(tab==='tasks'){
    const tasks=appState.tasks||[];
    box.innerHTML=`<div class="content-box"><h3>Beginner task board</h3>${tasks.length?tasks.map(t=>`<p>${t.completed?'✓':'□'} ${escapeHTML(t.title)} <small>(${t.duration} min)</small></p>`).join(''):'<p>Loading your tasks...</p>'}<button class="primary-btn" onclick="completeAllTasks()">Mark task set complete</button></div>`;
...
async function loadProgress(){
  try{
    ...
    const data=await response.json();
    if(data.success){
      appState.serverProgress=data.progress;
      appState.progress=data.progress.current_level||appState.progress;
      if(document.querySelector("#progressOutput")) document.querySelector("#progressOutput").textContent=appState.progress+"%";
      if(document.querySelector("#mainProgress")) document.querySelector("#mainProgress").style.width=appState.progress+"%";
    }
...
async function loadTasks(){
  try{
    ...
    if(data.success){
      ...
      if(appState.tab==="tasks") renderTab("tasks");
    }
  }catch(error){console.error(error);}
}
...
setFrequency(2);updateExplanation(1);renderTab('learning');animateBars();loadTasks();loadProgress();